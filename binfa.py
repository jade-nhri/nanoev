...
if '-i' in argv:
    ...
    filename=inputfile.split('/')[-1].replace('_clean.fa','')
    print (filename)
if '-o' in argv:
...
argv=sys.argv
thr=1000
rename(inputfile,thr)
os.chdir('../')
#####################################################
print('Get Feature.')
logfw.write('Get Feature.'+'\n')
...
fscfL=open('3_scf_order_by_column_long.txt','w');fscfS=open('3_scf_order_by_column_short.txt','w')
ffeaL=open('3_matrix_long.txt','w');ffeaS=open('3_matrix_short.txt','w')
for scf,fea in zip(fscf,ffea):
    ...
    if (scfLen>=scfThr) or (scf.strip() in CogList):
        fscfL.write(scf);ffeaL.write(fea)
        count+=1
    else:
        fscfS.write(scf);ffeaS.write(fea)
fscf.close();fscfL.close();fscfS.close()
ffeaL.close();ffeaS.close();fscf.close()
print(' %d contigs entering first stage clustering'%count)
fclr=open('3_clr_matrix.txt');fscf=open('3_scf_order_by_column.txt')
fclrL=open('3_clr_matrix_long.txt','w');fclrS=open('3_clr_matrix_short.txt','w')
...
fclr.close();fclrL.close();fclrS.close();fscf.close()
#####################################################
os.chdir('../../')
d=dict()
...
for i in dic.keys():
    strr = ''
    fw = open(outname+'/{0}-'.format(filename)+str(int(i)+1)+'_clean.fa','w')
    print('label',i)
    sp = dic[i].split(',')
    print(sp)
    for j in sp:
